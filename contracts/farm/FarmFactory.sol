...
contract FarmFactory is Ownable, IFarmFactory {
    ...
    IFarmReward public immutable override farmReward;
    ...
    constructor(
        string memory farmRewardName,
        string memory farmRewardSymbol,
        uint256 _rewardPerBlock,
        uint256 _startBlock
    ) {
        farmReward = new FarmReward(farmRewardName, farmRewardSymbol);
    ...
    function safeRewardTransfer(address to, uint256 amount) internal {
        uint256 balance = farmReward.balanceOf(address(this));
        if (amount > balance) {
            farmReward.transfer(to, balance);
        } else {
            farmReward.transfer(to, amount);
        }
    }
